...
final class RY_WSI_License extends RY_WSI_Model
{
    ...
    protected function valid_key(): void
    {
        ...
        if (!is_array($license_data)) {
            if (false !== $license_data) {
                $this->add_valid_error_log('WSI data error. ' . var_export($license_data, true)); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_var_export
                return;
            }
            $this->delete_license();
        ...
        if (!hash_equals($license_data['secret'], hash('crc32', $license_data['expire']))) {
            $this->add_valid_error_log('WSI secret check error.');
            return;
        }

        if ($license_data['expire'] <= time() - DAY_IN_SECONDS) {
            $this->add_valid_error_log('WSI expire. ' . $license_data['expire'] . ' @ ' . time());
            return;
        }

        if ($this->get_site_url() !== $license_data['url']) {
            $this->add_valid_error_log('WSI site change. ' . $license_data['url'] . ' => ' . $this->get_site_url());
            return;
        }
    ...
    {
        $license_key = RY_WSI::get_option('license_key', '');
        if (empty($license_key)) {
            $license_key = RY_WSI::get_option('pro_Key', '');
        }
        return $license_key;
    }
    ...
    public function set_license_data($data): void
    {
        ...
        RY_WSI::update_option('license_data', $data);

        RY_WSI::delete_option('pro_Data');
        RY_WSI::delete_option('pro_Key');
    }

    public function delete_license_key(): void
    {
        RY_WSI::delete_option('license_key');
        RY_WSI::delete_option('pro_Data');
        RY_WSI::delete_option('pro_Key');
    }
    ...
    {
        RY_WSI::delete_option('license_data');
    ...
    {
        $license_data = RY_WSI::get_option('license_data');
        if (empty($license_data)) {
            $license_data = RY_WSI::get_option('pro_Data');
            if (!empty($license_data)) {
                unset($license_data['type']);
                $license_data['url'] = $this->get_site_url();
                $license_data = base64_encode(serialize($license_data));
                RY_WSI::update_option('license_data', $license_data);
            }
        } else {
        ...
        if (empty($license_key)) {
            $license_key = RY_WSI::get_option('pro_Key', '');
            if (!empty($license_key)) {
                RY_WSI::update_option('license_key', $license_key);
            }
        }
    ...
    private function add_valid_error_log($message)
    {
        ...
        RY_WSI::update_option('license_auto_deactivate_date', current_time('Y-m-d'));
        RY_WSI_LicenseAutoDeactivate::add_note();
    }
}
